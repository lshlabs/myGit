...
class WizardStep2Dialog(QtWidgets.QDialog):
    ...
    def save_and_close(self):
        ...
        for label, rect in self.drag_areas.items():
            if rect:
                # 원본 이미지 전체를 복사한 후 해당 영역에 빨간 사각형 그리기
        ...
        # 모든 드래그 영역이 그려진 임시 이미지 저장 (temp_step2.png)
        ...
        if self.parent():
            parent = self.parent()
            parent.label_preview2.setPixmap(result.scaled(
                parent.label_preview2.size(),
            ...
            if all_areas_set:
                # 부모 창의 button_next2 활성화
                parent.button_next2.setEnabled(True)
                parent.button_next2.setStyleSheet(parent.button_next2.styleSheet().replace('darkgray', '#f0f0f0'))
    ...
    def update_preview(self):
        ...
        # 새로운 이미지에 드래그 영역 그리기
        result = scaled_pixmap.copy()  # 항상 새로운 복사본으로 시작
        painter = QPainter(result)
        painter.setPen(QPen(QColor('red'), 1))
