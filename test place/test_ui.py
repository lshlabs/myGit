...
def setup_ui(root):
    ...
    dummy_frame.grid(row=0, column=1, padx=5, pady=5, sticky='nesw')
    ...
    # 타이틀 라벨 생성 및 배치
    title_label = tk.Label(title_frame, text="배달의 민족", bg='#45D3D3', font=("default", 20, "bold"))
    ...
    frames = []
    for i in range(6):
        frame = tk.Frame(scrollable_frame, bg='blue', width=100, height=100)
    ...
    for i in range(50):
        label_row = 2 + i//3
        label_column = i%3
        tk.Label(scrollable_frame, text="라벨 " + str(i)).grid(row=label_row, column=label_column, pady=3)

    scrollbar.set(0,0)
    return content_canvas, frames, menu_frame, uni_frame, scrollbar, title_frame, title_label