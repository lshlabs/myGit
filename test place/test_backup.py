...
def save_data():
    ...
    if program_id == "배달의 민족":
        data[program_id] = {
            ...
            "entry_var1": entry_var1.get() if entry_var1.get() else None,
            "entry_var2": entry_var2.get() if entry_var2.get() else None
        }
    elif program_id == "요기요":
        data[program_id] = {
            "image_paths": image_paths,
        ...
        data[program_id] = {
            "image_paths": image_paths,
            "entry_var1": None,  # 만나의 경우 entry_var1이 없는 것으로 가정
            "entry_var2": entry_var2.get() if entry_var2.get() else None
        }

    # 윈도우 위치 저장
    data_window_position = {
        "window_geometry_main": root.geometry()
    }
...
def run_testplace():
    # 현재 파일의 디렉토리를 기준으로 testplace.py의 절대 경로를 구합니다.
    script_dir = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(script_dir, "testplace.py")
    
    # testplace.py 파일을 실행합니다.
...
if __name__ == "__main__":
    ...
    btn_setting.config(command=run_testplace)
