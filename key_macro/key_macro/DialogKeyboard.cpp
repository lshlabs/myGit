...
BOOL CDialogKeyboard::OnInitDialog()
{
	...
	FillComboBoxVkList (IDC_COMBO_KEYBOARD, LOBYTE(_item.keybd.scan_vk_code));
...
void CDialogKeyboard::OnKeybdEvent (BYTE vk_code, bool vk_pressed)
{
...
{
	if (nIDEvent == 1000) {
		GetDlgItem(IDC_COMBO_KEYBOARD)->PostMessage (CB_SETCURSEL, GetVkIndex (_vk_input));
...
void CDialogKeyboard::OnBnClickedOk()
{
	...
	if      (IsDlgButtonChecked (IDC_RADIO_KEY_3) == BST_CHECKED) _flags = 0x03;	// 누르고 떼기
	else if (IsDlgButtonChecked (IDC_RADIO_KEY_1) == BST_CHECKED) _flags = 0x01;	// 누르기
	else if (IsDlgButtonChecked (IDC_RADIO_KEY_2) == BST_CHECKED) _flags = 0x02;	// 떼기

	if (IsDlgButtonChecked (IDC_CHECK_HANGUL) == BST_CHECKED) _flags |= 0x10;	// 한글입력상태 체크
