...
void CListBoxEx::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if      (ctrl && nChar == 'A') KeyDown_CtrlA ();
	else if (ctrl && nChar == 'C') KeyDown_CtrlC ();
	else if (ctrl && nChar == 'X') KeyDown_CtrlX ();
	else if (ctrl && nChar == 'V') KeyDown_CtrlV ();
	else if (nChar == VK_DELETE)   KeyDown_Delete ();
...
void CListBoxEx::KeyDown_Delete ()
{
	int count = GetSelCount();
	CArray<int,int> arr;
