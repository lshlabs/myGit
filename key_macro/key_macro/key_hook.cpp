...
void OnRawKeyboard (RAWKEYBOARD &keyboard)
{
	...
	*/
	...
	if (g_keyDlg)        g_keyDlg  ->OnKeybdEvent (vk_code, vk_pressed);
	else if (g_mouseDlg) g_mouseDlg->OnKeybdEvent (vk_code, vk_pressed);
	else if (g_recDlg)   g_recDlg  ->OnKeybdEvent (vk_code, vk_pressed, scan_code);
	else if (g_macroDlg) g_macroDlg->OnKeybdMouseEvent (vk_code, vk_pressed);
}
...
void OnRawMouse (RAWMOUSE &mouse)
{
	...
	*/
	...
	if (g_mouseDlg)      g_mouseDlg->OnMouseEvent (pt.x, pt.y);
