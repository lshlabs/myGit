...
CDialogMouse::CDialogMouse(CWnd* pParent /*=NULL*/)
	: CDialogCommon(CDialogMouse::IDD, pParent)
{
	g_mouseDlg = this;
}
...
BOOL CDialogMouse::OnInitDialog()
{
	...
	FillComboBoxVkList (IDC_COMBO_KEY, g_ini.keyMousePosCapture);
...
void CDialogMouse::OnKeybdEvent (BYTE vk_code, bool vk_pressed)
{
	if (vk_code == g_ini.keyMousePosCapture && vk_pressed == true) {
...
void CDialogMouse::OnBnClickedOk()
{
	...
	if (IsDlgButtonChecked (IDC_CHECK_ABSPOS) == BST_CHECKED) _flags |= MOUSEEVENTF_MOVE;
	if (IsDlgButtonChecked (IDC_CHECK_WHEEL)  == BST_CHECKED) _flags |= MOUSEEVENTF_WHEEL;
	...
	if (IsDlgButtonChecked (IDC_RADIO_BUTTON_CLICK) == BST_CHECKED) button_state = 3;	// 마우스 버튼 클릭
	if (IsDlgButtonChecked (IDC_RADIO_BUTTON_DOWN)  == BST_CHECKED) button_state = 1;	// 마우스 버튼 누르기
	if (IsDlgButtonChecked (IDC_RADIO_BUTTON_UP)    == BST_CHECKED) button_state = 2;	// 마우스 버튼 떼기
...
void CDialogMouse::OnCbnSelchangeComboKey()
{
	...
	if (index != -1) {
		g_ini.keyMousePosCapture = GetVkCode(index);
	}
}
